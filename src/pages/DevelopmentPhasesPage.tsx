...
const DevelopmentPhasesPage: React.FC = () => {
  ...
  const stats = [{ value: '$1.2B', label: t('dev.stats.investment') }, { value: '577K', label: t('dev.stats.area') }, { value: '4,000+', label: t('dev.stats.beds') }, { value: '2030', label: t('dev.stats.completion') }];
  const phases = [
  ...
  const milestones = language === 'ar' ? [{ year: '٢٠٢١', event: 'وضع حجر الأساس' }, { year: '٢٠٢٢', event: 'مرحلة ما قبل الإطلاق' }, { year: '٢٠٢٣', event: 'الإطلاق الرسمي لكابيتال ميد' }, { year: '٢٠٢٥', event: 'استقبال أول المرضى' }, { year: '٢٠٣٠', event: 'تشغيل المدينة بالكامل' }] : [{ year: '2021', event: 'Project Groundbreaking' }, { year: '2022', event: 'Pre-Launch Phase' }, { year: '2023', event: 'Official CAPITALMED Launch' }, { year: '2025', event: 'First Patients Admitted' }, { year: '2030', event: 'Full City Operational' }];
        ...
        <section className="page-hero">
          <div className="container mx-auto px-6 relative z-10 text-center">
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="mb-5">
              <p className="text-accent text-sm font-medium mb-1">70%</p>
              <Progress value={70} className="max-w-xs mx-auto mb-1 [&>div]:bg-accent" />
              <p className="text-xs text-white/80">{t('dev.hero.progress')}</p>
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
              className="text-4xl md:text-5xl font-bold text-white mb-4">{t('dev.hero.title')}</motion.h1>
            <motion.p initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
              className="text-lg text-white/90 max-w-2xl mx-auto mb-8">{t('dev.hero.subtitle')}</motion.p>
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    <Badge variant={phase.statusType === 'completed' ? 'default' : phase.statusType === 'inProgress' ? 'default' : 'outline'} className={`text-xs ${phase.statusType === 'inProgress' ? 'bg-accent hover:bg-accent/90 text-accent-foreground' : ''}`}>{phase.status}</Badge>
                  </div>
                  <p className="text-accent text-sm font-medium mb-2">{phase.period}</p>
                  <p className="text-muted-foreground text-sm mb-3">{phase.desc}</p>
                  <ul className="grid grid-cols-2 gap-2">{phase.items.map((item, j) => (<li key={j} className="text-xs text-muted-foreground flex items-center gap-2">{phase.statusType === 'completed' ? <CheckCircle2 className="w-3.5 h-3.5 text-primary" /> : <Clock className="w-3.5 h-3.5 text-muted-foreground" />}{item}</li>))}</ul>
                </div>
              </motion.div>
            ))}
          </div>
